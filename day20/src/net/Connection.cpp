...
void Connection::buildHttpResponse(const HttpRequest& request)
{
    ...
    if (request.method() == HttpMethod::GET || request.method() == HttpMethod::POST) {
        ...
        if (request.path().find("/static/") == 0) {
            ...
            if (FileUtil::readFile(filePath, m_writeBuffer)) {
                ...
                m_writeBuffer = headers + m_writeBuffer;
                LOG_DEBUG("Worker %d serving static file: %s", m_workerId, filePath.c_str());
                return;
            } else {
        ...
        if (request.path() == "/chunked") {
            m_writeBuffer = response.buildFirstChunk();
            m_writeBuffer += response.buildChunk(body);
            m_writeBuffer += response.buildLastChunk();
            LOG_DEBUG("Worker %d sending chunked response %s to %s",
                      m_workerId, body, getClientInfo().c_str());
        } else {
