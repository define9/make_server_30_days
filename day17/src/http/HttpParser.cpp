...
size_t HttpParser::parseBody(const char* data, size_t len) {
    (void)data; // body data currently unused, tracking via content-length only
    ...
    // 本次调用最多消费 bodyLen 字节
    size_t bodyLen = len;
    // 防止超出 contentLength (m_bodyRead 是已消费的)
    if (m_bodyRead + bodyLen > static_cast<size_t>(contentLength)) {
        bodyLen = contentLength - m_bodyRead;  // 只消费到 body 结束
    }
    ...
    return bodyLen;  // 返回本次消费的数量，调用方据此裁剪 buffer
}