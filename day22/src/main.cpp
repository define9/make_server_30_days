...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        LOG_INFO("========================================");
        LOG_INFO("  Day22: Integration Testing");
        LOG_INFO("========================================");
        ...
        router.get("/api/users/:id", [](const HttpRequest& req, HttpResponse& resp) {
            ...
            resp.setBody("{\"message\": \"User profile\", \"path\": \"" + req.path() + "\"}");
        });
        ...
        router.get("/test/empty", [](const HttpRequest& req, HttpResponse& resp) {
            (void)req;
            resp.setStatus(HttpStatusCode::OK);
            resp.setContentType("text/plain");
            resp.setBody("Empty path test OK\n");
        });

        router.get("/test/big-header", [](const HttpRequest& req, HttpResponse& resp) {
            resp.setStatus(HttpStatusCode::OK);
            resp.setContentType("text/plain");
            std::string headerCount = std::to_string(req.headers().size());
            resp.setBody("Headers count: " + headerCount + "\n");
        });

        router.post("/test/chunked", [](const HttpRequest& req, HttpResponse& resp) {
            resp.setStatus(HttpStatusCode::OK);
            resp.setContentType("text/plain");
            resp.setBody("Received: " + req.body() + "\n");
        });

        router.get("/test/echo", [](const HttpRequest& req, HttpResponse& resp) {
            resp.setStatus(HttpStatusCode::OK);
            resp.setContentType("text/plain");
            resp.setBody("Query: " + req.query() + "\n");
        });

        LOG_INFO("Registered %zu routes", router.routeCount());
