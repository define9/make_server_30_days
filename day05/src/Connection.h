...
public:
    /**
     * @brief 构造函数 - 接受已连接的socket
     * @param fd 客户端socket描述符
     * @param addr 客户端地址
     * @note socket必须已经设置为O_NONBLOCK
     */
    ...
    /**
     * @brief 处理读事件 - ET模式下循环读取直到EAGAIN
     * @return 成功返回true，连接断开返回false
     */
    bool handleRead();

    /**
     * @brief 处理写事件 - ET模式下循环写入直到EAGAIN
     * @return 成功返回true，连接断开返回false
     */
    bool handleWrite();
     ...
     * @brief 是否有数据等待发送
     */
    bool hasDataToSend() const { return !m_writeBuffer.empty(); }

private:
    int m_fd;                  // socket描述符(非阻塞)
